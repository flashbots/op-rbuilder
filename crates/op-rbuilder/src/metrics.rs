...
pub struct OpRBuilderMetrics {
    ...
    /// Scheduler triggers that found a sealed continuous candidate ready.
    pub continuous_trigger_ready_total: Counter,
    /// Scheduler triggers that had to wait for the build task because no
    /// candidate had been sealed yet.
    pub continuous_trigger_miss_total: Counter,
    /// Time spent waiting for the build task after a trigger miss.
    pub continuous_trigger_fallback_wait_duration: Histogram,
