...
impl FlashtestationsBuilderTx {
    ...
    fn simulate_register_tee_service_tx<ExtraCtx: Debug + Default>(
        ...
        match result {
            ...
            ExecutionResult::Revert { output, gas_used } => {
                let revert_reason =
                    FlashtestationRegistryError::FlashtestationRegistryErrorErrors::abi_decode(
                        &output,
                    )
                    .unwrap();
                Ok(TxSimulateResult {
                    ...
                    success: false,
                    state_changes: state,
                    revert_reason: Some(revert_reason.into()),
                    logs: vec![],
                })
            }
    ...
    fn simulate_verify_block_proof_tx<ExtraCtx: Debug + Default>(
        ...
        match result {
            ...
            ExecutionResult::Revert { output, gas_used } => {
                let revert_reason =
                    BlockBuilderPolicyError::BlockBuilderPolicyErrorErrors::abi_decode(&output)
                        .unwrap();
                Ok(TxSimulateResult {
                    gas_used,
                    success: false,
    ...
    fn register_tee_service_tx<ExtraCtx: Debug + Default>(
        ...
        if success {
            ...
            FlashtestationRegistryError::FlashtestationRegistryErrorErrors::TEEServiceAlreadyRegistered(_),
        )) = revert_reason
        {
            Ok((None, true))
        } else {
