...
#[test]
fn test_scored_payload_transactions_no_rules_preserves_order() {
    ...
    let txs = vec![
        make_mock_tx(Address::random(), Address::random(), []),
        make_mock_tx(Address::random(), Address::random(), []),
        make_mock_tx(Address::random(), Address::random(), []),
    ];

    struct MockPayloadIter {
        txs: Vec<MockFbTransaction>,
        index: usize,
    }

    impl PayloadTransactions for MockPayloadIter {
        type Transaction = MockFbTransaction;
    ...
    let iter = MockPayloadIter {
        txs: txs.clone(),
        index: 0,
    };
    let mut scored = ScoredPayloadTransactions::new(iter, ruleset);

    // Without scoring rules, should preserve original order
    let mut collected = Vec::new();
    while let Some(tx) = scored.next(()) {
        collected.push(tx);
    }

    assert_eq!(collected.len(), txs.len());
    // Order should be preserved when no scoring rules
    for (i, tx) in collected.iter().enumerate() {
        assert_eq!(tx.hash(), txs[i].hash());
    }
}

#[test]
fn test_scored_payload_transactions_with_rules_reorders() {
    use crate::rules::ScoredPayloadTransactions;
    use reth_payload_util::PayloadTransactions;
    use std::sync::Arc;
    ...
    // Put low priority tx first
    let iter = MockPayloadIter {
        txs: vec![tx_low.clone(), tx_high.clone()],
        index: 0,
    };
    let mut scored = ScoredPayloadTransactions::new(iter, ruleset);
...
#[test]
fn test_scored_payload_transactions_empty_iterator() {
    use crate::rules::ScoredPayloadTransactions;
    use reth_payload_util::PayloadTransactions;
    use std::sync::Arc;
    ...
    struct EmptyIter;
    impl PayloadTransactions for EmptyIter {
        type Transaction = MockFbTransaction;
        fn next(&mut self, _ctx: ()) -> Option<Self::Transaction> {
            None
        }
    ...
    let mut scored = ScoredPayloadTransactions::new(EmptyIter, ruleset);
    assert!(scored.next(()).is_none());
}

#[test]
fn test_scored_payload_transactions_single_item() {
    use crate::rules::ScoredPayloadTransactions;
    use reth_payload_util::PayloadTransactions;
    use std::sync::Arc;
    ...
    struct SingleIter {
        tx: Option<MockFbTransaction>,
    }
    impl PayloadTransactions for SingleIter {
        type Transaction = MockFbTransaction;
        fn next(&mut self, _ctx: ()) -> Option<Self::Transaction> {
            self.tx.take()
        }
...
fn test_scored_payload_transactions_mark_invalid_delegates() {
    ...
    impl PayloadTransactions for TrackingIter {
        type Transaction = MockFbTransaction;
        fn next(&mut self, _ctx: ()) -> Option<Self::Transaction> {
            None
        }
    ...
    scored.mark_invalid(Address::random(), 0);
